...
- **「版本紀錄」與「強制更新推送內容」是同一份來源**：都讀 `frontend/src/lib/changelog.ts`。所以「更新版本紀錄」與「更新強制更新推送的內容」是**同一個動作**——只要在 changelog.ts 最前面新增一筆即可，不需要改兩個地方。
...
2. **整理這一版的更新內容（notes）**
   ...
   - 寫成**使用者看得懂的**幾條繁體中文短句（動作導向，非技術細節），風格與 changelog.ts 既有條目一致。
...
4. **更新 changelog**（`frontend/src/lib/changelog.ts`）
   - 在 `CHANGELOG` 陣列**最前面**新增一筆：
     ```ts
     { version: 'X.Y.Z', date: 'YYYY-MM-DD', notes: ['…', '…'] },
     ```
