...
> 最新版本與更新內容見 [GitHub Releases](https://github.com/playknightkoala/DietDiary/releases) 或 App 底部「版本紀錄」（來源：`frontend/public/changelog.json`）。前後端版號由 `scripts/bump-version.mjs` 保持一致，前端會輪詢 `GET /api/version` 對過舊的用戶端跳出強制更新。
...
- **逐頁記錄（照片頁＋無照片項目頁可混合）**：每張照片各自記份數與自定義熱量；沒拍到的食物可加「無照片項目」逐項記，之後補拍照片時已填內容自動跟著照片。
- **六大類與熱量**：11 個細分份數欄位（蛋豆魚肉低/中/高/超高脂、蔬菜、全穀雜糧、油脂堅果、水果、乳品脫脂/低脂/全脂）自動換算熱量，收斂成六大類顯示。
- **自定義熱量項目**：六大類無法表達的食物（含糖飲料、酒精等）可直接記熱量——糖（克 ×4）、酒精（毫升 ×7）、蛋白質（克 ×4）自動換算，或自訂名稱＋大卡；計入該餐與當日總熱量。
- **熱量及三大營養素**：主頁卡片依份數與自定義項目換算醣類／蛋白質／脂質公克數與熱量占比（甜甜圈圖），並累計精緻糖；每筆貼文也顯示該餐的三大營養素。
- **從歷史加入**：以「一餐」為單位的歷史卡片，可一鍵加入整餐（所有照片、份數、自定義與敘述）或只加單張；每餐別各顯示最近 30 餐，開啟時自動停在正在記錄的餐別分頁。
- **喝水／運動**：皆為逐筆記錄，一筆一則動態（含時間），可單筆刪除。
- **身體數據與歷程紀錄**：體重、體脂率、腰圍、肌肉重、體脂重，集中在獨立的「身體數據」視窗（右上角「更多功能」選單開啟）：當日數值、BMR／TDEE 與仿 InBody 報告的歷程趨勢一次呈現——每項指標一列、最近 8 次量測逐點標值。可**掃描 InBody 報告照片**由 AI 自動填入數值與檢測日期（需開放 AI；照片僅辨識、不儲存），並附前次量測對照。
- **BMR／TDEE**：在記錄視窗的「每日消耗量」分頁設定身高、出生年（年齡逐年自增）、性別、活動量與體重目標（減重／增重 ±300／±500／自訂大卡），依最近一次體重自動計算 BMR（Mifflin-St Jeor）與 TDEE，顯示於身體數據視窗。設定後 TDEE 同時成為主頁「今日攝取熱量」卡的每日熱量目標——顯示進度條與「距離目標還可吃多少」，超標以醒目色提醒。
- **介面自定義**：右上角「更多功能 → 介面自定義」可調整主頁各總覽卡片（今日攝取熱量／喝水／熱量及三大營養素／六大類份數）的順序與顯示與否；設定跟著帳號儲存，換裝置登入自動帶入。
- **跨裝置安心編輯**：同一筆餐點的儲存、刪除、照片增減都有樂觀鎖（revision）保護——內容已在其他裝置更新時會提示重新開啟，不會互相覆蓋；照片存取需登入且僅本人與營養師／管理者可見（fail-closed）。
- **階段目標與超標紅字**：可設定多組日期區間目標（六大類份數＋喝水）；當日某類 > 目標 × 1.2（目標為 0 時只要 > 0）即轉紅。營養師設定的目標會員不可自改。
- **月曆與週曆跳日**、內建**食物份數指南**。
- **社群互動**：每則動態可留言；營養師可對照片評分（紅黃綠燈）、調整份數（會員端顯示「調整前 → 調整後」逐值對照）、留言、替會員設定目標，營養師頁同時顯示會員的 BMR／TDEE 與當日熱量對比；有通知中心。
- **AI 營養師**（需管理者逐一開放）：拍照估份數＋自動寫敘述、InBody 報告辨識、單篇餐點評語、今日總評、對 AI 輸出按讚／倒讚回饋，並有跨使用者的「共用菜色知識庫」。
- **帳號系統**：Email 註冊（圖形驗證碼＋Email 認證碼）→ 管理者審核開通；JWT 登入；忘記密碼可自助重設（圖形驗證碼＋Email 認證碼），管理者也可於後台直接重置會員密碼；四種角色（member／citizen／dietitian／admin）。
...
│       ├── routes/           auth / days / entries / goals / trend / profile /
...
| `TAVILY_API_KEY` | — | Tavily 網路搜尋金鑰；**留空＝所有搜尋功能靜默停用** |
| `TAVILY_MONTHLY_BUDGET` | `950` | 每月搜尋次數上限（免費額度守門；快取命中不計次）|
...
| 類別 | 熱量 | 醣類 | 蛋白質 | 脂肪 |
|---|---|---|---|---|
| 蛋豆魚肉（低/中/高/超高脂） | 55 / 75 / 120 / 135 | 微量 | 7g | 3 / 5 / 10 / 10g↑ |
| 乳品（脫脂/低脂/全脂） | 80 / 120 / 150 | 12g | 8g | 微量 / 4 / 8g |
| 蔬菜 | 25 | 5g | 1g | — |
| 全穀雜糧 | 70 | 15g | 2g | 微量 |
| 油脂堅果 | 45 | — | — | 5g |
| 水果 | 60 | 15g | 微量 | — |

**自定義熱量換算**：糖 1g＝4 大卡（累計為「精緻糖」並計入醣類）、酒精 1ml＝7 大卡、蛋白質 1g＝4 大卡（計入蛋白質）；自訂項目自填大卡。酒精與自訂項目只計熱量、不計入三大營養素。
...
**BMR／TDEE**：BMR 採 Mifflin-St Jeor（9.99×體重＋6.25×身高−4.92×年齡＋166×性別−161，男=1 女=0）；TDEE＝BMR×活動量係數（1.2／1.375／1.55／1.725／1.9），再依體重目標（減重−／增重＋每日大卡）調整。基本資料存於後端 `/api/profile`，公式與係數同樣在 `domain.ts`（共用函式 `bmrTdeeOf`，身體數據視窗、「今日攝取熱量」卡與營養師頁的顯示都由它計算）。
...
AI 為**逐一開放**：管理者在後台替個別會員開啟後才可用；且需設定 `LLM_TOKEN`。六項能力：

1. **拍照判份數＋寫敘述**（`POST /api/ai/ocr`）：視覺模型估六大類份數並寫一句敘述；若共用知識庫命中相似菜色，會附上「社群參考份數」。敘述與份數可分別按讚／倒讚。
2. **InBody 報告辨識**（`POST /api/ai/inbody`）：拍下 InBody 報告，AI 讀出檢測日期／時間、體重、骨骼肌重、體脂肪重、體脂肪率、腹圍與身高／年齡／性別，自動填入記錄視窗並附前次量測對照；照片僅在記憶體辨識、不落地儲存。
3. **單篇 AI 評語**（`POST /api/ai/comment`）：針對一則餐點動態，比對當日階段目標給溫和建議；自定義熱量與三大營養素會一併納入參考。
4. **AI 今日總評**（`POST /api/ai/daily`）：綜合整天餐點、六大類達成、三大營養素、喝水、運動、身體數據；有設定 TDEE 時會拿當日總攝取與 TDEE 目標（含減重／增重調整）比較給建議。目標比對由後端先算好再交給模型，避免算錯。
5. **回饋**（`POST /api/ai/feedback`）：對 AI 輸出按讚／倒讚，會影響往後生成風格。
6. **營養查證**（`POST /api/ai/research`，限營養師／管理者）：輸入問題，AI 以網路搜尋結果整理成含來源出處的摘要。

**網路搜尋**（選用）：設定 `TAVILY_API_KEY` 後啟用——拍照辨識遇到市售品牌品項時自動搜尋營養標示換算份數、AI 評語／今日總評可自主發起一次搜尋查證、供上述營養查證使用。內建每月額度守門（預設 950 次，`TAVILY_MONTHLY_BUDGET` 可調）與 90 天結果快取；未設定金鑰時所有搜尋功能靜默停用、不影響其他功能。
...
- **忘記密碼**：登入頁「忘記密碼？」→ 圖形驗證碼 → Email 認證碼 → 設定新密碼（帳號不存在或未開通時靜默不寄信，避免帳號探測）。會員收不到信時，管理者可於後台直接重置密碼。
