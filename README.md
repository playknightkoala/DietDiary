...
> 目前版本 **1.3.2**。前後端版號由 `scripts/bump-version.mjs` 保持一致，前端會輪詢 `GET /api/version` 對過舊的用戶端跳出強制更新。
...
- **從歷史加入**：以「一餐」為單位的歷史卡片，可一鍵加入整餐（所有照片、份數、自定義與敘述）或只加單張；每餐別各顯示最近 30 餐。
- **喝水／運動**：皆為逐筆記錄，一筆一則動態（含時間），可單筆刪除。
- **身體數據與歷程紀錄**：體重、體脂率、腰圍、肌肉重、體脂重；「看趨勢」以仿 InBody 報告的歷程紀錄呈現——每項指標一列、最近 8 次量測逐點標值。可**掃描 InBody 報告照片**由 AI 自動填入數值與檢測日期（需開放 AI；照片僅辨識、不儲存），並附前次量測對照。
- **BMR／TDEE**：在記錄視窗的「每日消耗量」分頁設定身高、出生年（年齡逐年自增）、性別、活動量與體重目標（減重／增重 ±300／±500／自訂大卡），身體數據卡依最近一次體重自動計算 BMR（Mifflin-St Jeor）與 TDEE。設定後 TDEE 同時成為主頁「今日攝取熱量」卡的每日熱量目標——顯示進度條與「距離目標還可吃多少」，超標以醒目色提醒。
...
**BMR／TDEE**：BMR 採 Mifflin-St Jeor（9.99×體重＋6.25×身高−4.92×年齡＋166×性別−161，男=1 女=0）；TDEE＝BMR×活動量係數（1.2／1.375／1.55／1.725／1.9），再依體重目標（減重−／增重＋每日大卡）調整。基本資料存於後端 `/api/profile`，公式與係數同樣在 `domain.ts`（共用函式 `bmrTdeeOf`，身體數據卡與「今日攝取熱量」卡的目標都由它計算）。
