...
> 目前版本 **1.3.0**。前後端版號由 `scripts/bump-version.mjs` 保持一致，前端會輪詢 `GET /api/version` 對過舊的用戶端跳出強制更新。
...
- **BMR／TDEE**：在記錄視窗的「每日消耗量」分頁設定身高、出生年（年齡逐年自增）、性別、活動量與體重目標（減重／增重 ±300／±500／自訂大卡），身體數據卡依最近一次體重自動計算 BMR（Mifflin-St Jeor）與 TDEE。
...
**BMR／TDEE**：BMR 採 Mifflin-St Jeor（9.99×體重＋6.25×身高−4.92×年齡＋166×性別−161，男=1 女=0）；TDEE＝BMR×活動量係數（1.2／1.375／1.55／1.725／1.9），再依體重目標（減重−／增重＋每日大卡）調整。基本資料存於後端 `/api/profile`，公式與係數同樣在 `domain.ts`。
...
AI 為**逐一開放**：管理者在後台替個別會員開啟後才可用；且需設定 `LLM_TOKEN`。五項能力：
...
4. **AI 今日總評**（`POST /api/ai/daily`）：綜合整天餐點、六大類達成、三大營養素、喝水、運動、身體數據；目標比對由後端先算好再交給模型，避免算錯。
