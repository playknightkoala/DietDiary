...
export function BodyModal() {
  ...
  // 從身體數據總覽開啟：顯示「上一步」，完成／✕ 後回到總覽
  const logBodyReturn = useStore((s) => s.logBodyReturn);
  ...
  const finish = async () => {
    ...
    try {
      ...
      if (logBodyReturn) setModal('bodyView');
      else closeModal();
    }
  };
    ...
    <ModalShell maxWidth={420} cardStyle={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '90vh', overflowY: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
          {logBodyReturn && (
            <button
              onClick={() => !closing.current && setModal('bodyView')}
              className="hv-cream"
              title="返回身體數據（不儲存本次輸入）"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 3, border: '1.5px solid #DDD8CA', borderRadius: 99, background: '#fff', color: '#4A5A4A', fontSize: 12, fontWeight: 700, padding: '5px 12px 5px 8px', cursor: 'pointer', flex: 'none' }}
            >
